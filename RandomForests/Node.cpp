...
void Node::sortIndex(int featureId)
{
	...
	for(int i=0;i<_samples->getSelectedSampleNum();++i)//��feature��sample��Ӧ��sample�������?
	{
	...
	{sampleId[i]=pairs[i].id;}//��Ӱ��ԭ����
	delete[] pairs;
}

int compare_pair( const void* a, const void* b )//a < b,return -1,   = reutrn 0,   > return 1 
{
   Pair* arg1 = (Pair*) a;
   Pair* arg2 = (Pair*) b;
...
void ClasNode::calculateParams()//����_gini
{
	int i=0;
	int*sampleId=_samples->getSampleIndex();
	...
	for(i=0;i<classNum;++i)// prob��ʼ��Ϊ0
	{_probs[i]=0;}
	for(i=0;i<sampleNum;++i)
	{_probs[static_cast<int>(_samples->_labels[sampleId[i]])]++;}//��ѡ�е������ı�ǩ++  ������ñ�ǩ��ѡ�������г��ֵ�Ƶ��?
	for(i=0;i<classNum;++i)
	{
		float p=_probs[i]/sampleNum;//pΪ����
		gini+=(p*p);
	}
	_gini=1-gini;
}
